...
const MobileMenu = ({ setMobileOpen }) => {
  return (
    <nav className="flex flex-col mx-2 my-0 space-y-3 z-50 bg-white min-h-screen">
      <Link
        to=""
        className={`text-[17px] py-2 focus:text-orange-600 focus:bg-[#dbeafe] rounded-md pl-4 text-center pr-2 duration-300 `}
      >
        <button
      ...
      <Link
        to="allProducts"
        className={`text-[17px] py-2 focus:text-orange-600 focus:bg-[#dbeafe] rounded-md pl-5 text-center pr-2 duration-300 `}
      >
        <button
          onClick={() => setMobileOpen(false)}
          className="flex items-center justify-start gap-x-4"
